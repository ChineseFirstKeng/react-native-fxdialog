 ...
 * 设置为一个足够大的数值以避免文本被截断
 */
export const FXDialogMaxNumberOfLines = 999999;

/**
 * Dialog 在 FXView 系统中的分类标识
 */
export const FXDialogFXViewCategory = "Dialog";

/**
 * 弹窗类型枚举
 * 定义了三种不同的弹窗展示形式
 */
export enum FXDialogType {
  /** 居中弹窗 - 在屏幕中央显示的警告框 */
  Alert = "alert",
  /** 底部操作表 - 从底部滑出的操作选择列表 */
 ...
 * 定义了弹窗被关闭的各种系统触发方式
 */
export enum FXDialogCloseSystemType {
  /** 点击背景蒙层关闭 */
 ...
 * 定义了按钮的样式和行为类别
 */
export enum FXDialogActionType {
  /** 默认按钮样式 */
  Default = "default",
  /** 取消按钮样式 - 通常用于取消操作 */
  Cancel = "cancel",
 ...
 * 定义了弹窗显示和隐藏时的动画效果
 */
export enum FXDialogAnimationType {
  /** 无动画 - 立即显示/隐藏 */
  None = "none",
  /** 淡入淡出动画 */
  Fade = "fade",
...
export enum FXDialogContentKind {
  Title = "title",
  Message = "message",
 ...
 * 类型保护函数：判断是否为自定义内容
 */
export const checkIsCustomContent = (
  content: FXDialogContent,
): content is FXDialogContent => {
  return "content" in content;
};
export function resolveDialogContentKind(
  content: FXDialogContent,
): FXDialogContentKind | undefined {
  /**
   * 类型保护函数：判断是否为标题
   */
  const isTitle = (content: FXDialogContent): content is FXDialogTitle => {
   ...
   * 类型保护函数：判断是否为消息
   */
  const isMessage = (content: FXDialogContent): content is FXDialogMessage => {
  ...
  } else if (checkIsCustomContent(content)) {
...
export enum FXDialogActionKind {
  /**
   * 默认类型按钮
   */
  Default = "default",
...
export const checkIsDefaultAction = (
  action: FXDialogAction,
): action is FXDialogDefaultAction => {
  return action && typeof (action as FXDialogDefaultAction).action === "string";
};
export const checkIsCustomAction = (
  action: FXDialogAction,
): action is FXDialogCustomAction => {
  return action && typeof (action as FXDialogCustomAction).action !== "string";
};
export const resolveDialogActionKind = (
  action: FXDialogAction,
): FXDialogActionKind | undefined => {
  if (checkIsDefaultAction(action)) {
    return FXDialogActionKind.Default;
  } else if (checkIsCustomAction(action)) {
