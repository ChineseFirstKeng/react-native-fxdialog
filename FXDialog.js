...
class FXDialog {
     ...
     */
    static alert() {
        return this.createDialog(types_1.FXDialogType.Alert);
    }
    static actionSheet() {
        return this.createDialog(types_1.FXDialogType.ActionSheet, types_1.FXDialogAnimationType.SlideUp);
    }
    static popup() {
    ...
    show(fxViewId) {
        try {
            ...
            react_native_fxview_1.logger.error("[Dialog] Failed to show dialog", error);
            return null;
        }
    }
    /**
    ...
    static close(fxViewId, closeType) {
        try {
            react_native_fxview_1.logger.log("Dialog static close", fxViewId);
            FXDialogManager_1.default.getInstance().close(fxViewId, closeType || types_1.FXDialogCloseSystemType.Custom);
        }
        catch (error) {
            react_native_fxview_1.logger.error("[Dialog] Failed to close dialog", error);
        }
    }
    /**
     * 静态函数某个视图上的关闭所有弹窗
    ...
    static clearAll(fxViewId) {
        react_native_fxview_1.logger.log("Dialog clearAll");
        FXDialogManager_1.default.getInstance().clearViewController(fxViewId);
    }
}
exports.default = FXDialog;
